...
// -------------------------------------------------------------------
function initMap(){
  map = L.map('map', { zoomControl:true }).setView([44.4323, 26.1063], 11); // Setat implicit pe zona București / Ilfov
...
function switchToTab(panelId){
  document.querySelectorAll('.tab').forEach(t => t.toggle('active', t.dataset.panel === panelId));
...
async function onCourierAddressChange(input, which){
  ...
  if (result && result.outOfArea){
    ...
    showToast(`"${addr}" se localizează în afara zonei deservite.`, true);
  } else if (result){
...
async function geocodeAllPending(){
  ...
  for (let i = 0; i < pending.length; i++){
    ...
    // Respectăm regulamentul Nominatim API de rate-limiting (1 cerere pe secundă)
  ...
  renderAddresses();
  redrawMap();
  showToast('Procesul de localizare s-a finalizat.');
}
...
async function recalcRouteDistance(courierId){
  ...
  try {
    ...
    if (data.code === 'Ok' && data.routes.length > 0){
      ...
      // Adăugăm o durată fixă per oprire (ex: 5 minute de predare colet)
...
function initActionBar(){
  ...
  document.getElementById('resetBtn').addEventListener('click', () => {
    if (confirm('Sigur vrei să ștergi toate datele?')) {
      ...
      maybeShowGeocodeButton();
      redrawMap();
      showToast('Toate datele au fost resetate.');
    }
  });
...
function autoDistributeAddresses(){
  ...
  // Algoritm simplu de clustering geografic (repartizare la cel mai apropiat curier)
  ...
  // Ordonăm adresele în ordinea optimă a apropierii (Greedy TSP aproach)
  confirmedCouriers.forEach(c => {
...
function renderRouteSummary(){
  ...
  state.couriers.forEach(c => {
    ...
    r.order.forEach((addrId, idx) => {
      ...
      // Estimare vizuală timp sosire (include timpii de condus + 5 minute predare anterioară)
      if (idx > 0 && r.totalMin) {
        currentMin += (r.totalMin / r.order.length);
      } else {
        currentMin += 10; // distanța inițială aproximativă de la depozit
      }
  ...
  if (!hasAnyRoute) {
    container.innerHTML = '<div style="font-size:12px; color:#64748b; text-align:center; padding:20px;">Nu există trasee active calculate. Repartizează adresele din tab-ul Import.</div>';
  }
}
...
function redrawMap(){
  ...
  // 1. Randare Puncte Plecare Curieri
  state.couriers.forEach(c => {
    if (c.start.lat) {
      ...
      L.circleMarker(p, {
        ...
        weight: 2,
        fillOpacity: 1
      }).addTo(markersLayer).bindPopup(`<strong>${escapeHtml(c.name)} (Start)</strong><br>${escapeHtml(c.start.address)}`);
    }
  });

  // 2. Randare Adrese Clienți
  state.addresses.forEach(a => {
    if (a.lat) {
      const p = [a.lat, a.lng];
      bounds.push(p);
      ...
      L.circleMarker(p, {
        ...
        <strong>${escapeHtml(a.clientName || 'Client')}</strong><br>
        ${escapeHtml(a.raw)}<br>
        ${courier ? `Alocat la: <strong>${escapeHtml(courier.name)}</strong>` : 'Nerepartizat'}
      `);
    }
  });

  // 3. Randare Linii Trasee (Polylines OSRM)
  state.couriers.forEach(c => {
    const r = state.routes[c.id];
    if (r && r.geometry) {
      const poly = L.geoJSON(r.geometry, {
        style: { color: c.color, weight: 4, opacity: 0.85 }
...
function exportToExcel(){
  const rows = [[
    'Curier', 'Fereastră Orară', 'Nr. Comandă',
    'Prenume', 'Nume', 'Telefon',
    'Adresă Livrare', 'Detalii Locație',
    'Metodă Plată', 'Sumă de Plată', 'Notă Client'
  ]];
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'Trasee Optimizate');
  XLSX.writeFile(wb, `trasee_curieri_${new Date().toISOString().slice(0,10)}.xlsx`);
  showToast('Fișierul Excel a fost generat și descărcat.');
}
