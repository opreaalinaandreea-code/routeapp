...
function assignAddressesToNearestCourier(addrs, couriers){
  ...
  // reset only the free (non-manually-assigned) addresses
  ...
  // balance: account for manually-locked addresses too, so a courier with many manual
  // assignments doesn't also get overloaded with auto-assigned ones.
  const target = Math.ceil(addrs.length / couriers.length);
  let changed = true;
  let iterations = 0;
  while (changed && iterations < 50){
    ...
    for (const c of couriers){
      const assignedFree = free.filter(a => a.courierId === c.id);
      const totalAssigned = assignedFree.length + locked.filter(a => a.courierId === c.id).length;
      if (totalAssigned > target && assignedFree.length){
        // find a freely-assigned address in this group that is closest to some OTHER
        // courier's start (manually-locked addresses are never moved)
        let moveAddr = null, otherCourier = null, bestGain = -Infinity;
        assignedFree.forEach(a => {
          const distToOwn = haversine(a.lat, a.lng, c.start.lat, c.start.lng);
          couriers.forEach(oc => {
            if (oc.id === c.id) return;
            const ocTotal = free.filter(x => x.courierId === oc.id).length + locked.filter(x => x.courierId === oc.id).length;
            if (ocTotal >= target) return;
            const distToOther = haversine(a.lat, a.lng, oc.start.lat, oc.start.lng);
            const gain = distToOwn - distToOther;
            if (gain > bestGain){
              bestGain = gain; moveAddr = a; otherCourier = oc;
            }
          });
        });
        if (moveAddr && otherCourier && bestGain > 0){
          moveAddr.courierId = otherCourier.id;
          changed = true;
        }
      }
    }
